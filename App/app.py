...
background_image_url="https://smarttek.solutions/wp-content/uploads/artificial-in-healthcare-1170x516.jpeg"

page_bg_img = f"""
<style>
[data-testid="stAppViewContainer"] {{
background-image: url({'https://smarttek.solutions/wp-content/uploads/artificial-in-healthcare-1170x516.jpeg'});
...
models={
    'diabetes':pickle.load(open('Models/diabetes model.sav', 'rb')),
    'heart disease':pickle.load(open('Models/heart model.sav', 'rb')),
    'parkinsons':pickle.load(open('Models/Parkinsons model.sav', 'rb')),
    'lung cancer':pickle.load(open('Models/Lung model.sav', 'rb')),
    'thyroid':pickle.load(open('Models/thyroid model.sav', 'rb')),
}
...
if selected=='Diabetes Prediction':
    ...
    BMI = display_input("BMI", "Enter BMI value", "BMI", "number")
...
if selected=='Heart Disease Prediction':
    ...
    st.write("Enter the following details of the heart disease:")

    age=display_input("Age","Enter age of person","age","number")
    ...
    chol=display_input("Serum Cholestrol in mg/dl","Enter serum cholestrol","chol","number")
    ...
    exang=display_input("Exercise Induced Angina (1=yes;0=no)","Enter exercise induce angina","exang","number")
    oldpeak=display_input("ST depresion induced by exercise","Enter ST depression value","oldpeak","number")
    slope=display_input("Slope of the peak exercise ST segment (0,1,2)","Enter slope value","slope","number")
    ca=display_input("Major vessels covered by fluoroscopy (0-3)","Enter number of major vessels","ca","number")
    thal=display_input("Thal (0=normal;1=fixed defect;2=reversible defect)","Enter maximum heart rate","thal","number")
...
if selected=="Parkinson's Prediction":
    ...
    flo=display_input('MDVP:Flo(Hz)','Enter MDVP: Flo(Hz)','flo','number')
    ...
    Shimmer_dB=display_input('MDVP:Shimmer(dB)','Enter MDVP: Shimmmer(dB) value','Shimmer_dB','number')
    ...
    DDA=display_input('Shimmer:DDA','Enter MDVP: DDA value','DDA','number')
...
if selected=="Lung Cancer Prediction":
    ...
    WHEEZING=display_input('Wheezing (1=yes;0=no)','enter if the person has wheezing','WHEEZING','number')
    ...
    SHORTNESS_OF_BREATH=display_input('shortness Of Breath (1=yes;0=no)','enter if the person experienes shortness of breath','SHORTNESS_OF_BREATH','number')
    ...
    if st.button("Lung Cancer Test Result"):
        lung_prediction=models['lung cancer'].predict([[GENDER,AGE,SMOKING,YELLOW_FINGERS,ANXIETY,PEER_PRESSURE,CHRONIC_DISEASE,FATIGUE,ALLERGY,WHEEZING,ALCOHOL_CONSUMING,COUGHING,SHORTNESS_OF_BREATH,SWALLOWING_DIFFICULTY,CHEST_PAIN]])
...
if selected=="Hypo Thyroid Prediction":
    ...
    age=display_input('Age','Enter age of the person','age','number')
    sex=display_input('Sex (1=male;0=female)','Enter sex of the person','sex','number')
    on_thyroxine=display_input('On Thyoxine (1=yes;0=no)','Enter if the person is on_thyroxine','on_thyroxine','number')
    ...
    thyroid_diagnosis=''
    if st.button("Hypo Thyroid Prediction"):
        thyroid_prediction = models['thyroid'].predict([[age, sex, on_thyroxine, tsh, t3, tt4]])  
